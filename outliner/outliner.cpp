...
namespace qLibrary{
    namespace Graphics{
        ...
        bool checkOutlineAt(cimg_library::CImg<unsigned char> &orig,cimg_library::CImg<unsigned char> &out,int x,int y,bool chkStat){
            ...
            if((checkColorDivergence(currc,upperc) or checkColorDivergence(currc,lefterc)) and (!chkStat)){
                out(x,y,0,0)=255;
                out(x,y,0,1)=255;
                out(x,y,0,2)=255;
                return true;
